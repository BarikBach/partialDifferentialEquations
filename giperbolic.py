...
def explicit_finite_difference(a, t, h, l, T):
    u, analytical, xgird, ygird = giperbol(a, t, h, l, T)
    n = len(xgird)
    m = len(ygird)
    sigma = float(a * t**2 / (h**2))
    if sigma >= 1:
        print("Схема неустойчива")
    print("explicit")
    for k in range(2, n):  # T
        for j in range(1, m-1):  # X
            u[k, j] = (t * a ** 2 * (u[k-1, j+1] - 2 * u[k-1, j] + u[k-1, j-1])
                       / (h ** 2) + 2 * u[k-1, j] - u[k-2, j])
    for i in range(n):
        for j in range(m):
...
def implict_finite_difference(a1, t, h, l, T):
...
u, anT, x, y = explicit_finite_difference(0.025, 0.1, 0.1 * math.pi, math.pi, 1.)
fig = pylab.figure()
axes = Axes3D(fig)
axes.plot_surface(x, y, u, rstride=1, cstride=1, cmap = cm.jet)
pylab.show()
u, anT,x, y = implict_finite_difference(0.025, 0.1, 0.1 * math.pi, math.pi, 1.)
fig = pylab.figure()
axes = Axes3D(fig)
axes.plot_surface(x, y, u, rstride=1, cstride=1, cmap = cm.jet)
pylab.show()
